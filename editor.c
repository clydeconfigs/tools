/*
edit files
created by clyde (Sun, 28 Jan 2024 10:55:02 CET)
*/
...
char *RUBY_HL_keywords[] = {
    ...
    "unless",       "until",    "when",     "while", "yield",  NULL};
...
void editorUpdateSyntax(erow *row) {
    ...
    int singleline_comment_start_length =
        singleline_comment_start ? strlen(singleline_comment_start) : 0;
    int multiline_comment_start_length =
        multiline_comment_start ? strlen(multiline_comment_start) : 0;
    int multiline_comment_end_length =
        multiline_comment_end ? strlen(multiline_comment_end) : 0;

    int previous_separator = 1;
    int in_string = 0;
    int in_comment =
        (row->index > 0 && E.row[row->index - 1].highlight_open_comment);

    int i = 0;
    while (i < row->rendered_size) {
        char c = row->rendered_characters[i];
        unsigned char prev_highlight =
            (i > 0) ? row->highlight[i - 1] : HL_NORMAL;

        if (singleline_comment_start_length && !in_string && !in_comment) {
            if (!strncmp(&row->rendered_characters[i], singleline_comment_start,
        ...
        if (multiline_comment_start_length && multiline_comment_end_length &&
            !in_string) {
            if (in_comment) {
            ...
            } else if (!strncmp(&row->rendered_characters[i],
        ...
        if (E.syntax->flags & HL_HIGHLIGHT_NUMBERS) {
            if ((isdigit(c) &&
                 (previous_separator || prev_highlight == HL_NUMBER)) ||
        ...
        if (previous_separator) {
            ...
            for (j = 0; keywords[j]; j++) {
                ...
                if (!strncmp(&row->rendered_characters[i], keywords[j], klen) &&
                    ...
                    memset(&row->highlight[i], kw2 ? HL_KEYWORD2 : HL_KEYWORD1,
                           klen);
                    i += klen;
                    break;
                }
...
int editorRowFilePositionXToScreenPositionX(erow *row, int file_position_x) {
    ...
    for (j = 0; j < file_position_x; j++) {
        if (row->characters[j] == '\t')
            screen_position_x +=
                (EDITOR_TAB_STOP - 1) - (screen_position_x % EDITOR_TAB_STOP);
        screen_position_x++;
    }
...
int editorRowScreenPositionXToFilePositionX(erow *row, int screen_position_x) {
    ...
    for (file_position_x = 0; file_position_x < row->size; file_position_x++) {
        if (row->characters[file_position_x] == '\t')
            cur_screen_position_x += (EDITOR_TAB_STOP - 1) -
                                     (cur_screen_position_x % EDITOR_TAB_STOP);
...
void editorUpdateRow(erow *row) {
    ...
    row->rendered_characters =
        malloc(row->size + tabs * (EDITOR_TAB_STOP - 1) + 1);

    int index = 0;
    for (j = 0; j < row->size; j++) {
        if (row->characters[j] == '\t') {
            row->rendered_characters[index++] = ' ';
            while (index % EDITOR_TAB_STOP != 0)
                row->rendered_characters[index++] = ' ';
        } else {
...
void editorDelRow(int at) {
    ...
    memmove(&E.row[at], &E.row[at + 1],
            sizeof(erow) * (E.number_of_rows - at - 1));
...
void editorInsertNewline() {
    if (E.file_position_x == 0) {
        ...
        editorInsertRow(E.file_position_y + 1,
                        &row->characters[E.file_position_x],
...
void editorDelChar() {
    ...
    if (E.file_position_x > 0) {
        ...
        editorRowAppendString(&E.row[E.file_position_y - 1], row->characters,
                              row->size);
...
char *editorRowsToString(int *buflen) {
    int totlen = 0;
    int j;
    for (j = 0; j < E.number_of_rows; j++) totlen += E.row[j].size + 1;
    *buflen = totlen;
...
void editorFindCallback(char *query, int key) {
    ...
    if (saved_highlight) {
        memcpy(E.row[saved_highlight_line].highlight, saved_highlight,
               E.row[saved_highlight_line].rendered_size);
    ...
    for (i = 0; i < E.number_of_rows; i++) {
        ...
        if (match) {
            ...
            E.file_position_x = editorRowScreenPositionXToFilePositionX(
                row, match - row->rendered_characters);
            ...
            memset(&row->highlight[match - row->rendered_characters], HL_MATCH,
                   strlen(query));
            break;
        }
    }
...
void editorScroll() {
    ...
    if (E.file_position_y < E.number_of_rows) {
        E.screen_position_x = editorRowFilePositionXToScreenPositionX(
            &E.row[E.file_position_y], E.file_position_x);
    }
...
void editorDrawRows(struct abuf *ab) {
    ...
    for (y = 0; y < E.screen_rows; y++) {
        ...
        if (filerow >= E.number_of_rows) {
            if (E.number_of_rows == 0 && y == E.screen_rows / 3) {
                ...
                int welcomelen =
                    snprintf(welcome, sizeof(welcome),
                             "Editor editor -- version %s", EDITOR_VERSION);
            ...
            unsigned char *highlight =
                &E.row[filerow].highlight[E.column_offset];
            int current_color = -1;
            int j;
            for (j = 0; j < len; j++) {
                if (iscntrl(c[j])) {
...
void editorRefreshScreen() {
    ...
    snprintf(buf, sizeof(buf), "\x1b[%d;%dH",
             (E.file_position_y - E.row_offset) + 1,
...
void editorMoveCursor(int key) {
    erow *row = (E.file_position_y >= E.number_of_rows)
                    ? NULL
                    : &E.row[E.file_position_y];
    ...
    row = (E.file_position_y >= E.number_of_rows) ? NULL
                                                  : &E.row[E.file_position_y];
...
void initEditor() {
    ...
    if (getWindowSize(&E.screen_rows, &E.screen_columns) == -1)
        die("getWindowSize");
    E.screen_rows -= 2;
}
